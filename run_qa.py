...
def main():
    ...
    if args.do_train:
        ...
        Data_path='../data/train_features.pickle'
        ...
        train_dataloader=cycle(train_dataloader)  #死循环
    ...
    if args.do_test:
        ...
        convert_func_eval = functools.partial(convert_examples_to_features,
                             ...
                             max_question_length=args.max_question_length,
                             is_training=3)
        Data_path='../data/test_features_{}.pickle'.format(args.k) #前K个文档的
        if os.path.exists(Data_path):
            with open(Data_path, 'rb') as tf:
                test_features=pickle.load(tf)
        else:
            with Pool(50) as p:
                test_features = p.map(convert_func_eval, test_examples)
            with open(Data_path, 'wb') as tf:
                pickle.dump(test_features,tf)
