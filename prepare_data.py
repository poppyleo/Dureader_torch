...
def read_examples(input_file, doc_file, is_training, k=1, es_index="passages", es_ip="localhost"):
    ...
    # test
        ...
        examples=[]
        for val in df[['id', 'docid', 'question']].values:
            docids=val[1].split(',')
            for docid in docids:
                passage = doc[doc['docid'] == docid].values[0][-1]
                examples.append(InputExample(
                    guid=val[0],
                    docid=docid,
                    text_a=passage,
    ...
    while i < len(selected_sentenses):
        ...
        if len(selected_sentenses[i]['tokens'])+len(doc_tokens) <= max_doc_length:
            ...
            if i == len(selected_sentenses)-1:
                ...
                # if multi_sentense_end > multi_sentense_start:
                #     print(multi_sentense_end,multi_sentense_start)
                #     raise EOFError
...
class Result(object):
    ...
    def update(
        ...
        logits = torch.Tensor([[(start_preds[i][d[0]]*end_preds[i][d[1]]) if self.is_valid_index(examples[i], d) else -1e3 for d in indices[i]] for i in range(cls_logits.size(0))])
...
#             and int(torch.max(class_preds,dim=1)[1][i])==0: # predict 'known'
    ...
    def score(self):
        ...
        pred_dict=dict(zip(example_id,prediction))
        if not self.is_testing:
            #验证集
            true_dict = json.load(open('/home/none404/hm/baidu_qa/'+ 'dureader_robust-data/dev.json'))
            F1, EM, TOTAL, SKIP = evaluate(true_dict, pred_dict)
            return pred_dict,(F1,EM)
        else:
     ...
     if len(array)<=1:
         ...
         for  i in array:
             if i >=mid_num:
                 ...
                 maxlist.append(i)
     return mysort(minlist)+[mid_num]+mysort(maxlist)
